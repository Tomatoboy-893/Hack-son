...
export default function InstructorAvailabilityScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (error.code) { // Firebaseエラーコードとメッセージをログ
        console.error("Firebase Error Code (取得):", error.code);
        console.error("Firebase Error Message (取得):", error.message);
      }
  ...
  const handleAddSlot = async () => {
    ...
    try {
      const availabilityRef = collection(db, 'skills', skillId, 'availability');
      // デバッグログで保存しようとしているinstructorIdとcurrentUserIdを確認
      console.log("handleAddSlot: 保存しようとしているinstructorId:", currentUserId);
      console.log("handleAddSlot: ログイン中のユーザーUID (auth.currentUser.uid):", auth.currentUser?.uid);
      console.log("handleAddSlot: 保存パス:", `skills/${skillId}/availability`);
      ...
      await addDoc(availabilityRef, {
        ...
        instructorId: currentUserId, // これがFirestoreのresource.data.instructorIdになる
      });
        ...
        { text: "キャンセル", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              // デバッグログで削除しようとしているデータパスを確認
              console.log("handleDeleteSlot: 削除パス:", `skills/${skillId}/availability/${slotId}`);
              console.log("handleDeleteSlot: ログイン中のユーザーUID (auth.currentUser.uid):", auth.currentUser?.uid);
              ...
              if (error.code) { // Firebaseエラーコードとメッセージをログ
                console.error("Firebase Error Code (削除):", error.code);
                console.error("Firebase Error Message (削除):", error.message);
              }
